...
//	TOUCH_LOG("[S]-reportedFinger:%d\n", reportedFinger);
	for (i = 0; i < pReadData->count; i++) {

	/*	TOUCH_LOG("rf : cnt[%d/%d] id[%d] x[%d] y[%d] ma[%d] mi[%d] o[%d] p[%d] t[%d]\n",
			i,
			pReadData->count,
...
//	TOUCH_LOG("report:%d, new:%d, c:%d , p:%d, r:%d\n", reportedFinger, newFinger, changedFinger, pressedFinger, releasedFinger);
...
static ssize_t store_lpwg_notify(struct i2c_client *client, const char *buf, size_t count)
{
	...
	TOUCH_LOG("[KW_Debug] pDriverData->currState:%d / nextState:%d\n", pDriverData->currState, nextState);
...
static ssize_t show_lpwg_data(struct i2c_client *client, char *buf)
{
	...
	TOUCH_LOG("pDriverData->reportData.knockCount : %d\n", pDriverData->reportData.knockCount);
	for (i = 0; i < pDriverData->reportData.knockCount; i++) {
		TOUCH_LOG("Cordinates x[%d]:[%d] y[%d]:[%d]\n", i, pDriverData->reportData.knockData[i].x, i, pDriverData->reportData.knockData[i].y);
