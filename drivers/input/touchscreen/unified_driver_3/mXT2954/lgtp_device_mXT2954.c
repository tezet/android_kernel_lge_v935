...
static const char defaultFirmware[] = "atmel/t1lte_global_com/t1_0713v21.fw";
#elif defined(CONFIG_MACH_MSM8974_T1WIFI_GLOBAL_COM)
static const char defaultFirmware[] = "atmel/t1wifi_global_com/t1_0713v21.fw";
#elif defined(CONFIG_MACH_MSM8974_T1WIFIN_GLOBAL_COM)
static const char defaultFirmware[] = "atmel/t1wifin_global_com/t1_0713v21.fw";
#endif
...
static ssize_t mxt_version_show(struct i2c_client *client, char *buf)
{
	int ret = 0;
...
static ssize_t show_mxt_info(struct i2c_client *client, char *buf)
{
	...
	ret += sprintf(buf+ret, "=======[Additional Information]========\n");
...
static LGE_TOUCH_ATTR(version, S_IRUGO | S_IWUSR, mxt_version_show, NULL);
...
static int mXT2954_Initialize(struct i2c_client *client)
{
	...
	ts->vcc_dig = regulator_get(&client->dev, "vcc_dig"); /* why we don't use vdd_dig? */
...
static int mxt_proc_message(struct mxt2954_ts_data *ts, u8 *message, TouchReadData *pData)
{
	...
	#if 1 // For Atmel debugging - 15.06.17
	if(ts->debug_enabled) {
	...
	if (type == MXT_GEN_COMMAND_T6) {
		...
		if (pDriverData->isSuspend == false && !slimport_is_check()) {
...
static int mXT2954_lpwg_control(struct i2c_client *client, TouchState newState)
{
	...
	switch (newState) {
	case STATE_NORMAL:
		TOUCH_LOG("[Debug] Normal State - TapCount[%d]\n", ts->g_tap_cnt);
		ts->mxt_knock_on_enable = false;
		ts->mxt_multi_tap_enable = false;
	...
	case STATE_OFF:
		TOUCH_LOG("[Debug] State Off - TapCount[%d]\n", ts->g_tap_cnt);
		ts->mxt_knock_on_enable = false;
		ts->mxt_multi_tap_enable = false;
