 ...
 */
	...
	case MSM_CAMERA_LED_LOW:
		...
		pr_info("__debug: voltage = %d\n", voltage);
		if(voltage <= FLASH_LOW_VOLTAGE) {
			pr_err("Batter voltage is too low!!! Skip pre flash\n");
			return 0;
		}
#endif
		if (fctrl->torch_trigger) {
...
#if defined(CONFIG_MACH_MSM8974_DZNY_DCM)
		if(voltage <= FLASH_LOW_VOLTAGE) {
			pr_err("TBatter voltage is too low!!! Skip main flash\n");
			return 0;
		}
#endif
		if (fctrl->torch_trigger)
